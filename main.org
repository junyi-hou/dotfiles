...
By default, ~visual-fill-column-mode~ places the text on the left side of the window. I want to adjust the margin so the text is in the center of the window.
...
I use an ~emacs~ commit before 01/26/2021, as it is before a broken changes on child frame border is made. I put the relevant ~emacs-overlay~ version below:

#+begin_src nix :noweb-ref emacsOverlayVersion
  emacsOverlayHash = "a1c89c95cf659512f72f6ec51c764c03801f890d";
#+end_src
