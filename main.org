...
For MacOS, a new APFS volume may be needed before running the command above. See [[https://duan.ca/2020/12/13/nix-on-macos-11-big-sur/][this article]] for more information. Alternatively, use
...
- add project management configuration to ~org-mode~
...
I use ~flake~ to manage my configuration files. In particular, I define a new output schema ~homeModules~ which automatically imports all ~.nix~ file in the ~./modules/~ folder (and in the current git tree). Those modules then can be imported by ~home-manager~.

Modules are divided into 4 categories: ~home-manager-modules~, ~common-services~, ~linux-modules~, and ~darwin-modules~. The first two categories contain modules that should be applied to both Linux and MacOS systems. Whereas ~linux-modules~ are modules meant to be used in only Linux system, and ~darwin-modules~ are modules meant to be used in only MacOS.

The reason why I have a separate category for services that should be applied to both Linux and MacOS is because Linux (in particular ~home-manager~) and MacOS uses different init systems to launch services. So for those services to work on MacOS, I have to define them as ~nix-darwin~ modules rather than ~home-manager~ modules.
...
~home-manager~ cannot manage display sessions. This is fair as ~home-manager~ should not be used to install system-wide package and start system services. Instead, I use system package manager (~pacman~ in my case) to install relevant packages and services. On Linux/GNU system, the list of packages I installed with ~pacman~ includes

+ display services (~xorg-server~, ~xorg-drivers~, ~xorg-xinit~, ~i3gaps~, and ~i3status~)
+ sound service (~alsa-utils~)
...
#+begin_src nix :noweb-ref linux-modules :noweb yes
  {
    home = {
      file = {
...
#+begin_src nix :noweb-ref linux-modules
  {
...
MacOS uses ~launchd~ as its init system while ~home-manager~ only supports ~systemd~ init system. To enable services like ~emacs daemon~, ~mbsync~, and ~gpg-agent~, I include ~nix-darwin~, which is a package designed to tweak MacOS system settings.

#+begin_src nix :noweb-ref flake-input
  nix-darwin = {
...
Below is a list of services that needs extra configuration on MacOS:
- [[Package Management][~emacs~ daemon]]
- [[epa][gpg-agent]]
- [[Email Client][periodic mbsync update]]

~nix-darwin~ can also be used to customize MacOS options like enabling tapping on the trackpad. I include the customization below:

#+begin_src nix :noweb-ref darwin-module
  system.defaults = {
    dock = {
      autohide = true;
      mru-spaces = false;  # do not rearrange desktop space
      orientation = "left";
      tilesize = 32;
    };
...
This section contains the code and explanation of my personal configuration for ~emacs~ editor.

** ~emacs~ Package Management

I use a subflake to manage ~emacs~ and its packages. This flake defines 2 functions ~emacsPackage~ and ~emacsConfig~. ~emacsConfig~ takes no argument and returns a ~home-manager~ module with all ~emacs~-related configuration, while ~emacsPackage~ takes an ~emacs~ derivation as the only argument and returns a ~home-manager~ module

The advantage of using a flake to manage ~emacs~ packages over the more conventional methods (e.g., via ~package.el~ or ~straight.el~) is twofold. First, it provides a clean and intuitive way of pinning package versions. This allows reproducible development environment over several machines. Second, subflake allows me to integrate the ~emacs~ package management workflow with my general configuration management process.

There are two disadvantages of the current approach. First, the mechanism of pinning ~emacs~ package version is not perfect. In particular, packages inherit from ~epkgs~ are pinned to the version contained in ~epkgs~. This mechanism prohibits updating a single package while keeping the versions of other packages from ~epkgs~. The only way to update a package from ~epkgs~ is to update the whole ~epkgs~ repository.

To circumvent this issue, I define packages that needs separate updates using ~emacs.pkgs.trivialBuild~ or ~emacs.pkgs.meplaBuild~ to define packages from flakes input source. In this way, I can update a specific package using ~nix flake lock --upcate-input~ while keeping other packages pinned to the same version.

The second disadvantage of the current approach is its inflexibility. Since all package management functionalities are now handled by nix flake, I won't be able to run a simple ~(use-package foo)~ to quickly install ~foo~ package to try out. Instead, I need to modify ~/emacs.d/flake.nix~ to include the new package, run ~nix build .~ and restart ~emacs~ to enable the new package.

Below includes the structure of the subflake for all ~emacs~-related configuration. Since ~home-manager~ and ~nix-darwin~ have different API for installing packages (~home.packages~ vs ~environment.systemPackages~), I use a hack to include ~emacs~ output directly in the ~$PATH~ variable in order to make ~emacs~ available.

#+begin_src nix :tangle ./emacs.d/flake.nix :noweb yes
  # this file is auto-generated by main.org, do not modify!
...
Import this subflake alongside with ~emacs-overlay~ from ~nix-community~ to obtain derivations for the latest git version of ~emacs~.
...
Changes made in the ~emacs-module~ subflake will not automatically made available to the main flake due to the lock file. As a result, I specifically ask the main flake to update subflake before building.

#+begin_src shell :noweb-ref update-user-config-hook
  nix flake lock --update-input emacs-module
#+end_src

Enable ~emacs~ in both Linux and MacOS.
...
#+begin_src nix :noweb-ref darwin-modules
  (
    let
      pkgs = import nixpkgs { inherit system; overlays = [ emacs-overlay.overlay ]; };
...
Finally, include the configuration files
...
If I do need a fresh ~*Message*~ buffer, I can use the following function (bound in [[messages-buffer-mode]] section) to clear the content of the ~*Message*~ buffer.
...
~dash.el~, ~f.el~, ~s.el~ provides unified libraries to list, file and string manipulations. They may not be more efficient than the builtin ~seq.el~, ~file.el~ and ~string.el~, but they offer a consistent interface for almost all functions. [[https://yoo2080.wordpress.com/2014/02/23/threading-macros-from-dash-for-emacs-lisp/][This article]] provides a good explanation on ~dash.el~ and friends.

Register those libraries with nix flake:

#+begin_src nix :noweb-ref emacs-packages
  epkgs.dash epkgs.s epkgs.f
#+end_src

Enable them in ~emacs~:
...
~visual-fill-column-mode~ does not play well with ~poly-mode~, which enables multiple major modes in a same buffer. If there are both ~text-mode~ and ~prog-mode~ running in the same buffer, ~visual-fill-column-mode~ will try to reset the column width every time the cursor moves from ~text-mode~ blocks to ~prog-mode~ blocks (and vice versa). Therefore, I enable ~visual-fill-column-mode~ only when ~poly-mode~ is not present:
...
I also add a key to refresh the ~*Message*~ buffer.
...
I like the color theme to provide enough visual cue (to read code) but not too colorful and distracting.
...
To make this configuration portable, I define a font specifically for ~emacs~. In particular, the font consists of
+ ~PragmataPro~ for monospace ASCII/ligature;
+ ~SourceHanSans~ for CJK supports;
+ ~nerdfonts~ for glyphs not contained in ~PragmataPro~.

To install fonts, first register source files in ~flake~.
...
Then install it using ~home-manager~. Note that the vanilla ~home-manager~ do not have a module to install fonts. There is a [[https://github.com/nix-community/home-manager/issues/520#issuecomment-451759097][comment]] that adds a custom font module to ~home-manager~, which I include below.

#+begin_src nix :noweb-ref font-config
  (
...
With the help of this custom module, I can include fonts (and their configurations) in ~home-manager~ configuration.

#+begin_src nix :noweb yes :tangle ./modules/font.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
Since ~fontconfig~ is available in both Linux and MacOS, make font configuration universally available.
...
ligature.el is not available on ~elpa~ or ~melpa~ yet, so I need to build it from source.

#+begin_src nix :noweb-ref emacs-flake-input
  ligature = {
...
Different font offers different ligatures, and I need to register the proper ligatures that are compatible with the font that I am using. I follow the example configuration for ~Pragmata Pro~ in the [[https://github.com/mickeynp/ligature.el/issues/8#issuecomment-685015141][ligature repository]] to register ligatures.
...
Hide ~eldoc-box~ frame 0.5 seconds after I move the cursor. Also enable hiding it using ~C-g~
...
**** center at cursor after moving to end of buffer.
...
Overriding ~consult-outline~ use unexposed internal function ~consult--read~, so I asks ~use-package~ to add it to the autoload list of ~consult~ package.

#+begin_src emacs-lisp :tangle no :noweb-ref consult-autoloads
  (consult--read)
#+end_src

It is also useful to call ~consult-line~/~consult-outline~ during ~evil-search~. To do so, first make sure that ~evil-search-module~ is set to ~isearch~ (meaning ~evil-search~ will use ~isearch~ functionality)

#+begin_src emacs-lisp
  (setq evil-search-module 'isearch)
#+end_src

Then define commands that takes current ~isearch-string~ and put it in ~consult-line~/~consult-outline~.
...
#+begin_src conf :tangle ./emacs.d/etc/eshell/aliases :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
#+begin_src nix :noweb-ref cli-module-extra
  {
...
#+begin_src nix :noweb yes :tangle ./modules/vterm.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
Building ~vterm~ also requires some build essentials. I rely on the system version of ~libtool~, ~gnu make~ etc. for now due to [[https://github.com/akermu/emacs-libvterm/issues/333][this bug]]. Hopefully it will get resolved from the upstream.

#+begin_src nix :noweb-ref vterm-module-extra
  {
...
#+begin_src emacs-lisp :noweb-ref vterm-config :tangle no
...
Enable ~vterm~ in MacOS and Linux.
...
Historically, ~org-mode~ grew out of ~outline-mode~, which is used to fold text parts. ~org-mode~ inherit this functionality. Yet, the folding in ~org-mode~ depends on ~overlays~, which can be very slow. [[https://github.com/yantar92/org/tree/6afe70e63509214b4c305f2d8f2d713056b165ed][this repository]] re-implement the folding mechanism by using ~text properties~.
...
Using ~org-indent~ creates ugly blanks in front of the wrapped lines in src-blocks as discussed [[https://emacs-china.org/t/truncate-lines/17317][here]]. The following function fix this display issue.
...
Make ~org-block~, ~org-block-begin-line~ and ~org-block-end-line~ faces look better.
...
*** todo management with org-agenda and org-capture

The original purpose of the ~org-mode~ is to maintain to-do lists and plan projects. I collect settings related to such functionalities here.

First define the life cycle of a todo item (or a project):
...
Next, define keybindings for toggling priority and todo states.
...
I use the builtin ~org-agenda~ to manage my todo items. First, set up the folder that holds all ~org-todo~ files.

#+begin_src emacs-lisp
  (setq org-agenda-files `(,(expand-file-name "~/org-agenda/")))
#+end_src

By default, ~org-agenda~ rearranges my window configuration, which is not ideal and needs to be overridden.

#+begin_src emacs-lisp
  (setq org-agenda-window-setup 'other-window)
#+end_src

~org-agenda~ is launched in its own mode: ~org-agenda-mode~. I put this mode in the ~motion~ state and override its key bindings.
...
Next, provide an ~org-capture~ template specifically for adding todo item.
...
I ask ~org-capture~ not to mess with my current window configuration:
...
Finally, define global entry point for interacting with ~org-agenda~ and ~org-capture~.
...
Org-mode automatically renders links like [[org-appear][this]]. ~org-appear~ expands those links when the cursor is on them to make editing them easier. The master branch of ~org-appear~ does not support the ~fold~ feature branch I currently use as discussed [[https://github.com/awth13/org-appear/issues/5][here]]. Instead, ~feature/org-fold-support~ adds such support and thus I will use this branch for now.
...
I also want to turn off ~eldoc~ integration and use ~flymake-childframe~ instead.
...
With the help of ~ligagture.el~ and ~Pragmata Pro~ fonts, I could have fancy prefix to indicate the type of syntax issue/error that I have.
...
Finally, [[prescient]] provides a nice sorting mechanism for ~company~ depend on selection history and other stuffs.
...
I put ~completing-read~ on top to ~yas-prompt-functions~ so I can use [[selectrum]] in ~yas-choose-value~, which does not require match by default.
...
*** COMMENT unified documentation-displaying mechanism: help-childframe

I rely mostly on [[helpful mode]] and [[LSP: eglot]] to provide documentation. I wrote a small package ~help-childframe~ to display documentation retrieved by those backends in a child frame.
...
Enable ~help-childframe-mode~ globally.

#+begin_src emacs-lisp
  (add-hook 'after-init-hook #'global-help-childframe-mode)
#+end_src

*** incremental syntax tree parsing: tree-sitter

~tree-sitter~ provides a powerful and super fast incremental syntax parsing engine. ~emacs-tree-sitter~ implements ~tree-sitter~ parsing tools in ~emacs~. I first define the structure of ~tree-sitter~ package below.

#+begin_src nix :noweb-ref emacs-packages :noweb yes
  (
    let
      <<tree-sitter-component>>
    in
      <<tree-sitter-final-package>>
  )
#+end_src

~emacs-tree-sitter~ actually contains 3 different packages: ~tsc.el~, ~tree-sitter~, and ~tree-sitter-langs~. Installation of ~tsc.el~ and ~tree-sitter-langs~ involve downloading binaries from release page. As nix build environment does not contain network access, I need to modify the build scripts to install those packages.

I first install ~tsc.el~. During build process, ~tsc.el~ will download a architecture-specific library ~tsc.so/tsc.dylib/tsc.dll~, which is included only in the release tarball. I override the source of the default ~tsc.el~ package and customize the build script to install the library files in addition to the usual ~emacs~ library ~*.el~.
...
I also need to disable the fetching step in the installation phase so that nix would not complaint about not able to download.

#+begin_src diff :tangle ./emacs.d/tree-sitter-patches/tsc-do-not-download.patch :mkdirp yes
  diff --git a/tsc.el b/tsc.el
  index 717705c..ceb7543 100644
  --- a/tsc.el
  +++ b/tsc.el
  @@ -26,8 +26,9 @@
   (eval-and-compile
...
Finally, I modify the build script:

#+begin_src nix :noweb-ref tree-sitter-component
...
Replace the builtin ~tsc.el~ dependency with the modified version.

#+begin_src diff :tangle ./emacs.d/tree-sitter-patches/tree-sitter-remove-buildin-tsc-dependency.patch :mkdirp yes
  diff --git a/lisp/tree-sitter.el b/lisp/tree-sitter.el
  index a854bda..4787b99 100644
...
Apply this patch when building ~tree-sitter~ and make sure ~tsc.el~ is in its build environment.
...
Finally, install ~tree-sitter-langs~ from source.
...
Two changes must be made. First, remove the fetching code from installation script, and add back ~tsc~ and ~tree-sitter~ dependencies.

#+begin_src diff :tangle ./emacs.d/tree-sitter-patches/tree-sitter-langs-do-not-require-bundled-grammars.patch :mkdirp yes
  diff --git a/tree-sitter-langs.el b/tree-sitter-langs.el
  index 2d4468c..7baf2c2 100644
...
Since this custom version of ~tree-sitter-langs~ does not download bundled grammars, I have to build grammars separately.

#+begin_src nix :noweb-ref tree-sitter-component :noweb yes
...
Combine ~tree-sitter-langs~ and the grammars built above. Note that instead of symlinking the ~.so~ libraries to ~tree-sitter-langs~, I hard-code the ~tree-sitter-langs-grammar-dir~ variable to ~tree-sitter-grammar~ as suggested [[https://github.com/NixOS/nixpkgs/issues/108089][here]]. The reason for this hack is that ~tree-sitter~ seems unable to follow symlink when searching for grammar libraries, which leads to an ~Module is not GPL compatible~ error as mentioned in [[https://github.com/ubolonton/emacs-tree-sitter/issues/100][this issue]].

I also put in a ~tree-sitter-package-extra~ field in case I need any other tree-sitter-related packages.

#+begin_src nix :noweb-ref tree-sitter-final-package :noweb yes
  pkgs.symlinkJoin {
    ...
    paths = [
      ...
      tree-sitter-grammar
      <<tree-sitter-package-extra>>
    ];
  }
#+end_src

Finally, load ~tree-sitter~ and ~tree-sitter-langs~ in ~emacs~:
...
~tree-sitter~ can be enable by adding ~tree-sitter-mode~ to the relevant mode hook.

On its own, ~tree-sitter-mode~ does not provide any direct benefits since all it does is parsing the syntax tree. ~tree-sitter-hl-mode~ builds on top of ~tree-sitter-mode~ and provide high-performance syntax highlight. I enable ~tree-sitter-hl-mode~ whenever ~tree-sitter-mode~ is enabled.

#+begin_src emacs-lisp :tangle no :noweb-ref tree-sitter-hook
  (tree-sitter-mode . tree-sitter-hl-mode)
#+end_src

~tree-sitter~ can also be used to fold code. ~tree-sitter-fold~ package can be use to achieve this goal.
...
#+begin_src nix :noweb-ref tree-sitter-package-extra
  (emacsPkg.pkgs.trivialBuild {
...
I enable ~tree-sitter-fold-mode~ whenever ~tree-sitter-mode~ is enabled.
...
The builtin ~comint~ mode lacks some features important for data scientists. For example, there is no easy way to display graph in the REPL buffer. ~jupyter~ provides a nice REPL platform for almost all data-science-y languages that addresses those shortcomings of ~comint~.

There are two attempts to integrate ~jupyter~ with ~emacs~: the first is the ~ein~ package, which provides a fully-fledged ~jupyter~ notebook frontend. the second is the ~emacs-jupyter~ package, which is less ambitious in its scope and provide only the REPL thought a dedicated REPL mode (that works like ~jupyter-console~) and an org-babel integration. Since I do not need the fancy features of the notebook, I use the ~emacs-jupyter~ package.
...
First, according to the [[https://github.com/nnicandro/emacs-jupyter#org-mode-source-blocks][documentation]], I need to load all languages that I want to use jupyter kernel for first, and then ~jupyter~ at the last position, so it can overshadow some function defined in those libraries. To do so, I need to add any missing kernels to ~org-babel-load-languages~ before ~jupyter~ if I find the kernel available in the current local environment. As I alluded to in [[org-babel]], I do not need to take it out if those kernels become unavailable in other environments.
...
#+begin_src nix :noweb yes :tangle ./modules/grammar.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
#+begin_src nix :noweb-ref grammar-module
  {
    home.packages = with pkgs; [
...
Make grammar modules available everywhere.
...
First, I include ~GPG~ in my system.

#+begin_src nix :tangle ./modules/gpg.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
Some extra efforts are needed in order to enable ~gpg-agent~ service on MacOS.
...
Finally, enable ~gpg~ modules in both Linux and MacOS.
...
#+begin_src nix :tangle ./modules/pass.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
Finally, enable ~pass~ modules in both Linux and MacOS.
...
#+begin_src nix :noweb yes :tangle ./modules/ssh.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
Include server location shortcuts.

#+begin_src nix :noweb-ref ssh-servers
  "aws" = {
...
#+begin_src nix :noweb yes :tangle ./modules/git.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
~magit~ will hide ~recent commit~ section if there are commits not pushed onto the remote branch. I want ~recent commit~ section to be visible at all time.
...
#+begin_src nix :noweb-ref git-module-extra
  {
...
For the first two features, I use ~projectile~.
...
By default, ~projectile-find-file~ will ask me to switch to an known project if there is none. Instead of this behavior, I want it to call ~find-file~ instead.
...
Make sure that the builtin ~project.el~ recognize ~projectile~'s way of finding project root.
...
~projectile~ can utilize ~fd~ for faster file search. Enable it requires installing the ~fd~ utility.
...
For the last feature, I use ~deadgrep~ to search for text in projects. ~deadgrep~ use ~ripgrep~ as the backend to search text in project based on regexp.
...
#+begin_src nix :tangle ./modules/direnv.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
Make sure ~direnv~ is enabled in both Linux and MacOS.
...
+ ~mbsync~ (registered as ~isync~ in many package managers) to fetch and sync maildir with the remote server.
  ...
  1. import the original ~cyrus_sasl~ from ~nixpkgs~, since it is a dependency of all xoauth2 plugins
  ...
  3. add an overlay of ~cyrus_sasl~ with ~xoauth2~ libraries symlinking to ~${cyruss_sasl}/lib/sasl2~
     ...
     Until [[https://github.com/NixOS/nixpkgs/issues/108480][the upstream]] figured out some ways to deal with this, I will use my hack to allow ~mbsync~ retrieve emails from google. To do so, register the relevant packages. Note that in order to use this, make sure that ~google-api/id~ and ~google-api/secret~ are present in the storage path of [[https://www.passwordstore.org/][GNU Pass]].
     ...
     Then include them in the ~home-manager~ configuration.

     #+begin_src nix :noweb yes :tangle ./modules/mail.nix :mkdirp yes
       # this file is auto-generated by main.org, do not modify!
  ...
  In addition, include the following configuration for macOS to enable mail fetching service.
...
+ ~notmuch~ to index and search emails based on tag and other maildir attributes. It also provides a great ~emacs~ package to interact and manipulate its database.
  ...
  #+begin_src nix :noweb-ref mail-module-extra
    {
      programs.notmuch = {
        enable = true;
...
+ on its own, ~notmuch~ do not associate the maildir of mails with their tags. For example, mails tagged with ~trash~ are not moved to the ~trash~ maildir. ~afew~ deals with this problem by providing a "mover" command to move mails according to their tags. In addition, it provides an easy way to setup initial tagging.
  ...
  - for mails sent to a mailing list, add ~list~ tag and a tag indicating which mailing list the mail belongs to. Furthermore, archive mails from mailing list that are not on my "allow list".
    ...
    #+begin_src python :noweb-ref OnlyAllowListFilter
      import os
      import json
    ...
    #+begin_src nix :noweb-ref mail-module-extra :noweb yes
      {
...
Make sure this setting is picked up in both Linux and MacOS systems.
...
#+begin_src nix :noweb yes :noweb-ref account-setting
  accounts.email = let
...
#+begin_src nix :noweb-ref account-setting
...
#+begin_src nix :tangle ./modules/calendar.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
I use the ~calfw~ forked by ~tumashu~. This fork makes two improvements over the stack version. First, it uses the preferred ~cl-lib.el~ to replace the deprecated ~cl.el~. Second, it confirms the namespace conventional for ~emacs~ packages.
...
Register ~slack~. Like ~calfw~, the Slack package comes with some hard-coded faces.
...
#+begin_src emacs-lisp :tangle no :noweb-ref ielm-config
...
#+begin_src emacs-lisp :tangle no :noweb-ref ielm-init
...
Python is a general purpose language with many libraries that enable people to jump on a task without worrying too much about inventing the wheel.
...
Indentation is part of the syntax, and it must be 4. Also, do not spam me with the "guess indent offset" message.
...
Also set ~tab-width~ for all python buffers.

#+begin_src emacs-lisp :tangle no :noweb-ref python-init
  (defun gatsby:python--set-indent-width ()
    (setq-local tab-width 4))
#+end_src

Enable LSP, ~tree-sitter~ mode, and the local tab width when loading python files.
...
Format python source code buffer before saving using ~eglot~
#+begin_src emacs-lisp :tangle no :noweb-ref python-config
...
#+begin_src emacs-lisp :tangle no :noweb-ref python-binding
  (:states '(motion normal visual)
...
Register and install python grammar for the tree-sitter:
...
#+begin_src nix :noweb-ref tree-sitter-grammar
  tree-sitter-python-grammar
#+end_src

Enable tree-sitter when loading python file.
...
Installation of STATA should be done outside of this configuration, since STATA is closed source. I include below only the editing support for STATA script file.

*** STATA:major-mode

I'd like to use ~emacs~ to replace STATA's builtin IDE. To do so, I need to be able to send codes from my ~emacs~ to an underlying STATA process. ~stata_kernel~ implements a STATA kernel for jupyter REPL, for which I can interacting from within ~emasc~ using the [[emacs-jupyter]] package.
...
#+begin_src emacs-lisp :tangle no :noweb-ref stata-config
  (add-to-list 'gatsby:jupyter-repl-function-alist '(stata-mode . "stata"))
#+end_src

Setup the keybinding in STATA-mode just as other modes utilizing ~emacs-jupyter~.

#+begin_src emacs-lisp :tangle no :noweb-ref stata-binding
  (:states '(motion normal visual)
...
~R~ is an open source statistical package, which makes it attractive to people outside of economists. ~emacs~ can edit ~R~ files.
...
Instead of using the ~ess-r-mode~ to run ~R~ REPL, I use ~emacs-jupyter~ for REPL. To do so, I first register ~irkernel~ with ~gatsby:jupyter-repl-function-alist~.

#+begin_src emacs-lisp :tangle no :noweb-ref r-config
  (add-to-list 'gatsby:jupyter-repl-function-alist '(ess-r-mode . "irkernel"))
#+end_src

Setup keybindings for using ~emacs-jupyter~.

#+begin_src emacs-lisp :tangle no :noweb-ref r-binding
...
~R~ has two special operators, assign ("~<-~") and pipe ("~%>%~"). For the former, ~ess~ has a builtin ~ess-insert-assign~ function. For the latter, I define my own function to do that.
...
I bind ~ess-insert-assign~ and ~gatsby:r-pipe~ both to ~M-RET~, with a helper function ~gatsby:r-assign-or-pipe~ to determine which one should be applied based on whether there is a ~=~ before the point.
...
Make sure to run ~eglot~ along with R to enable the language server when editing R files.

#+begin_src emacs-lisp :tangle no :noweb-ref r-hook
  (ess-r-mode . eglot-ensure)
#+end_src

*** R:tree-sitter

Compile ~R~ grammar from source.
...
Register it with ~tree-sitter-langs~.
...
Enable ~tree-sitter-mode~ when loading ~R~ files.
...
#+begin_src emacs-lisp :tangle no :noweb-ref nix-config
...
#+begin_src emacs-lisp :tangle no :noweb-ref nix-init
...
Nix community offers a basic LSP server for linting ~.nix~ file. I first register it with ~flake~.
...
... and install using ~home-manager~,

#+begin_src nix :noweb yes :tangle ./modules/nix-development.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!

  inputs: with inputs; system: { pkgs, ... }: {
    home.packages = [
      rnix-lsp.defaultPackage."${system}"
      <<nix-development-extra-package>>
    ];
  }
#+end_src

... include it in both Linux and MacOS:

#+begin_src nix :noweb-ref home-manager-modules
  (self.homeModules."nix-development.nix" system)
#+end_src

... finally hook it onto the ~eglot~ server program so it can recognize and run LSP server automatically when I open a .nix file.

#+begin_src emacs-lisp :tangle no :noweb-ref nix-init
  (add-to-list 'eglot-server-programs '(nix-mode . ("rnix-lsp")))
#+end_src

#+begin_src emacs-lisp :tangle no :noweb-ref nix-hook
(nix-mode . eglot-ensure)
#+end_src

In addition, I use
#+begin_src nix :noweb-ref nix-development-extra-package
  pkgs.nixpkgs-fmt
#+end_src
to format my nix code before save.
...
~nix-format-buffer~ moves point to ~point-min~. This behavior is annoying since I hook ~nix-format-buffer~ to the ~before-save-hool~, which means I have to move back to the last-modified position after each save. The following advice fix this issue.
...
Download and compile nix grammar from source.
...
Register the nix grammar with ~tree-sitter~ in ~emacs~.
...
Writing ~nix~ expression could be time-consuming. A common scenario requires inserting variables defined in ~let...in~ binding in the corresponding scope/body. ~yasnippet~ snippets does not support modifying disjoint sections of text in the current buffer. Therefore, I implement separate functions to achieve this effect.

First, the following functions add/modify the text identified by regexp/tree-sitter syntax tree. By calling this function inside snippets, I can achieve the desired effect.
...
The first modifier function is regexp-based instead of tree-sitter-based. This is due to constant failed tree-sitter queries. I suspect that the tree-sitter syntax tree did not get updated instantaneously after buffer changes, which leads to failed queries. Ideally, I can switch to  tree-sitter-based identifiers in the future, so the unit of edit is syntax node rather than line.

That being said, tree-sitter is still useful for identifying the current syntax node, which serves as eligibility check for snippet expansions.
...
I then define the snippet functions below.
...
#+begin_src emacs-lisp :tangle no :noweb-ref man-config
...
I use AUCTeX to edit tex source file in ~emacs~. AUCTeX is a gigantic package that aims at providing a tex IDE for ~emacs~. Although I already use [[LaTeX:LSP][a LSP]] to handles label, autocomplete, etc. I still need AUCTeX to conveniently interact with tex command line utilities (mainly via ~TeX-command-sequence~ function)
...
AUCTeX tries to be smart and increase the font size of (sub)section titles, move up or down the super/subscripts, etc. I do not particularly appreciate such efforts:

#+begin_src emacs-lisp :tangle no :noweb-ref tex-custom-face
...
#+begin_src emacs-lisp :tangle no :noweb-ref tex-custom
  (tex-fontify-script nil)
  (font-latex-fontify-script nil)
  (font-latex-fontify-sectioning 1.0)
#+end_src

Load ~TeX-PDF-mode~, which allows me to use ~PDFTeX~ as the main tex engine.

#+begin_src emacs-lisp :tangle no :noweb-ref tex-hook
  (LaTeX-mode . TeX-PDF-mode)
#+end_src

Make sure that I enable synctex search and shell-escape.

#+begin_src emacs-lisp :tangle no :noweb-ref tex-config
...
First install ~texlive~ and the LSP via ~flake~. I use a light-weighted LSP ~digestif~. I use the ~master~ branch of the ~digestif~ package.
...
Make ~texlive~ and ~digestif~ available system-wide.

#+begin_src nix :noweb yes :tangle ./modules/latex.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
Make sure to load LaTeX module in both Linux and MacOS.
...
#+begin_src emacs-lisp :tangle no :noweb-ref tex-init
...
I use ~Zathura~ as my pdf viewer.

#+begin_src nix :noweb-ref latex-module-extra :noweb yes
  {
    programs.zathura = {
      enable = true;
      <<zathura-options>>
...
#+begin_src emacs-lisp :tangle no :noweb-ref tex-binding
...
~Zathura~ supports inverse search via default keybinding ~<C-left-button>~, and it seems there is [[https://unix.stackexchange.com/questions/423091/how-to-remap-an-existing-keybinding-in-zathura][no way to change this binding]]. Therefore, I bind a custom script that first move the cursor to the center of the ~zathura~ window. It then sends a ~<C-left-button>~ event to ~Zathura~ to conduct inverse search. I named this script ~zathuraInverseSearch~.

#+begin_src nix :noweb-ref zathura-options
  options = {
    synctex = true;
...
Unfortunately, ~zathuraInverseSearch~ is system-specific. In Linux, I use ~xdotool~ to mimic mouse movement and ~i3-msg~ to detect the current window setup. In MacOS, I use the builtin ~AppleScript~ to accomplish the same goal.

The Linux version goes:
...
# TODO: implement zathuraInverseSearch in macos

Apart from the ability of folding sections, enabling ~outline-minor-mode~ also allows me to navigate quickly between different sections.
...
First the updating script.

#+begin_src shell :shebang "#!/usr/bin/env nix-shell" :tangle update-user-config :noweb yes
  ...
  <<update-user-config-tangle-p>>

  <<update-user-config-hook>>
  ...
  if [ -f "result/activate" ]; then
      ...
      ./result/activate
  elif [ -f "result/sw/bin/darwin-rebuild" ]; then
      echo "found nix-darwin activation script, activating..."
      ./result/sw/bin/darwin-rebuild switch --flake .
  fi
#+end_src

Allowing the ~--no-tangle~ optional flag, which skips tangling ~.nix~ and ~.el~ from ~main.org~ and use the configuration as is.

#+begin_src shell :noweb-ref update-user-config-tangle-p
  if [ "$1" == "--no-tangle" ]; then
...
Then the within-emacs updating function.
...
The script to update ~README.org~.

#+begin_src shell :tangle update-readme :tangle-mode (identity #o755)  :shebang "#!/usr/bin/env sh"
...
Automatically update ~README.org~ when updating the whole configuration.
...
Here includes a series of small command line utilities.

#+begin_src nix :noweb yes :tangle ./modules/cli.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
I am keeping a close eye on [[https://github.com/akirakyle/emacs-webkit][emacs-webkit]] as a modern JS compatible browser inside ~emacs~, but until then, I will use ~firefox~ instead.
...
I include ~alacritty~ in case ~emacs~ fails.

#+begin_src nix :tangle ./modules/terminal-emulator.nix :mkdirp yes :noweb yes
  # this file is auto-generated by main.org, do not modify!
...
~alacritty~ uses ~OpenGL~ to improve performance. However, ~OpenGL~ applications installed by nix in non-nixOS Linux systems have a [[https://pmiddend.github.io/posts/nixgl-on-ubuntu/][bug]]. [[https://github.com/guibou/nixGL][nixGL]] wraps ~OpenGL~ libraries and provides a solution.
...
Override ~alacritty~ package in Linux with ~nixGL~.
...
I use [[https://drop.com/buy/drop-alt-mechanical-keyboard][drop-alt keyboard]], which supports custom layout via QMK firmware. To enable the custom layout, I first configure and download the layout file [[https://drop.com/mechanical-keyboards/configurator][here]]. The official firmware loader ~mdloader~ can be used to refresh the firmware. [[https://drop.com/talk/10343/how-to-configure-your-alt-keyboard][Here]] is a step-by-step tutorial.

The firmware loader supports both linux and macOS platform. However, the pre-built binary does not confirm with ~flake~ input format. Therefore I need to build it from source. To do so, first register ~mdloader~ with ~flake~.
...
Then make sure it is installed by the ~home-manager~.

#+begin_src nix :tangle ./modules/drop-keyboard-loader.nix :mkdirp yes
  # this file is auto-generated by main.org, do not modify!
...
Make the loader available in Linux and MacOS.

#+begin_src nix :noweb-ref home-manager-modules
  self.homeModules."drop-keyboard-loader.nix"
#+end_src

To refresh the keyboard firmware, connect it to the computer in DFU mode and run

#+begin_src shell
  mdloader --first --download /path/to/firmware/bin/file --restart
#+end_src

* TODO
** vterm: iron out quirks in integrating with ~evil~
