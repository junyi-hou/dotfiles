...
#+begin_src nix :noweb yes :tangle ./modules/direnv.nix :mkdirp yes
  inputs: with inputs; {
    <<nix-direnv-overlays>>
...
There is a [[https://github.com/nix-community/nix-direnv/issues/70][bug]] in the current version of the ~nix-direnv~ cannot support the ~strict_env~ setting in ~direnv~, as it will leave ~NIX_BIN_PREFIX~ variable unset. The following snippet taken from the same discussion fix this.

#+begin_src nix :noweb-ref nix-direnv-overlays
  nixpkgs.overlays = [
    (final: prev: {
...
Register STATA with ~flake~ and install it using ~home-manager~.
...
Note that I only include STATA package for ~linux~. There is a separate version of STATA for ~macOS~.
...
First install ~textlive~ and the LSP via ~flake~. I use ~texlab~ instead of more light-weighted ~digestif~ since the latter works only in ~linux~ but not ~macOS~.
