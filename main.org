...
#+begin_src nix :tangle ./linux/homeConfig.nix :mkdirp yes :noweb yes
  { pkgs, lib, config, ... }: {
...
#+begin_src nix :tangle ./darwin/homeConfig.nix :mkdirp yes :noweb yes
  { pkgs, lib, config, ... }: {
    programs.home-manager.enable = true;
    imports = [
      <<darwin-modules>>
...
#+begin_src nix :noweb-ref flake-linux-config
  {
    programs.emacs = {
      enable = true;
...
#+begin_src nix :noweb-ref flake-darwin-config
  (
    let
...
#+begin_src emacs-lisp
  ...
  The exact configuration to be built is determined by the output of \"uname -a\". This flake currently support x86_64-linux and x86_64-darwin platforms.
  "
    (interactive)
