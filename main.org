...
The following snippet initialize my ~home.nix~ file that feed into ~home-manager switch~ command that updates my dotfiles. It contains two fields:

- ~<<version-info>>~ to control package versions (e.g., python version)
- ~<<home-manager>>~ to import relevant home-manager configurations from different segment.
...
#+begin_src nix :noweb-ref version-info
...
Python is a general purpose language with many libraries that enable people to jump on a task without worrying too much about inventing the wheel. I use version 3.9 and 2.7 of python.

#+begin_src nix :noweb-ref version-info
  python3 = pkgs.python39;
  python2 = pkgs.python27;
...
#+begin_src nix :noweb-ref home-manager
  {
    home.packages = with pkgs; [
      (python3.withPackages (p: with p; [ jupyter xlrd openpyxl pandas ]))
...
#+begin_src nix :noweb-ref home-manager
  (
    let
      stata = with pkgs; stdenv.mkDerivation {
        name = "stata-14";
      ...
      stataKernel = with pkgs; python3.pkgs.buildPythonPackage rec {
        pname = "stata_kernel";
        version = "1.12.2";
    ...
    in
      ...
      {
        home = {
          packages = [ stata stataKernel ];

          file = {
            ".stata_kernel.conf".text = ''
              [stata_kernel]
...
#+begin_src nix :tangle ./system-specific-config.nix :noweb yes
  { pkgs, lib, config, ...}:
...
Make sure to load those configuration in the main ~home.nix~ file.

#+begin_src nix :noweb-ref home-manager
  ./system-specific-config.nix
#+end_src
