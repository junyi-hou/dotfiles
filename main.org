# -*- gatsby:is-config-source-file: t; -*-
...
Normally, I need to run ~home-manager switch~ in the command line to update dotfiles declared in ~home.nix~. In addition, since all my configuration are inside this org file, I need to first ~tangle~ them to generate new to generate new ~home.nix~. This means that to update my dotfiles, I need to finish a two step procedure, and in different places as well (first inside ~emacs~ to tangle, then in shell to switch).

To simplify this process, I define a function ~gatsby:switch-home~, which runs a custom script ~update-user-config~ asynchronously and pop up a buffer ~*home-manager*~ to run and report the output. I then hook this function after ~org-babel-tangle~ finishes. To make sure that this process runs only after I tangle *this* file, I add a file-local variable ~gatsby:is-config-source-file~, with value ~t~ in none but this file.

#+begin_src emacs-lisp :tangle no :noweb-ref home-manager-switch-hook
  (defcustom gatsby:is-config-source-file nil
    "Non-nil means to run `home-manager' related functions before and after `org-babel-tangle'."
    :type 'boolean
    :group 'org)

  (put 'gatsby:is-config-source-file 'safe-local-variable #'booleanp)

  (defun gatsby:switch-home (&rest _)
    "Make an asynchronous process running `home-manager switch', and output information to `*home-manager*' buffer.
  This function should not bu ran manually."
    (when gatsby:is-config-source-file
      (when (process-live-p "home-manager")
        (user-error "Another home-manager process is currently running, exiting"))
      (if (buffer-live-p (get-buffer "*home-manager*"))
          (with-current-buffer "*home-manager*"
            (let ((inhibit-read-only t))
              (delete-region (point-min) (point-max))))
        (with-current-buffer (get-buffer-create "*home-manager*")
          (special-mode)))
      ...
      (pop-to-buffer "*home-manager*")))

  (advice-add #'org-babel-tangle :after #'gatsby:switch-home)
#+end_src

The script ~update-user-config~ is also used to initialize configuration for the first time (see [[Overview]]). This script does two things. First, it runs any commands that might need to be ran before ~home-manager~ (e.g., generating nix-expression of ~node.js~ packages via ~node2nix~). Second, it runs ~home-manager switch~ to implement changes in ~home.nix~.
...
~org-refile~ functions can be used to update ~README.org~ alongside with the whole ~dotfile~ repository.
...
Automatically update ~README.org~ when updating ~dotfile~.

#+begin_src shell :noweb-ref update-user-config-hook
  if command -v emacs &> /dev/null; then
      ./update-readme main.org README.org
  else
      echo "emacs not found in PATH, skip updating README."
  fi
#+end_src

* ~home-manager~ Initialization

The following snippet initialize my ~home.nix~ file that feed into ~home-manager switch~ command that updates my dotfiles.
    ...
    home = {
      ...
      activation = {
        createProjectDir = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
          $DRY_RUN_CMD mkdir -p $VERBOSE_ARG ${projectDir}
        '';
        createResearchDir = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
          $DRY_RUN_CMD mkdir -p $VERBOSE_ARG ${researchDir}
        '';
      };
    };
  ...
  (
    ...
    in
...

      {
        ...
        config = {
          home.packages = cfg.fonts ++ (if config.fonts.fontconfig.enable then [ pkgs.fontconfig ] else []);
...
With the help of this module, I can now configure the fonts for ~emacs~.
#+begin_src nix :noweb-ref home-manager :noweb yes
  (
    let
      pragmataPro = with pkgs; stdenv.mkDerivation {
        pname = "PragmataPro";
        version = "0.828";
        src = /home/lactaid/Downloads/PragmataPro0.828-2.zip;
...
Add ~aws~ host for testing.

#+begin_src nix :noweb-ref host-alias
  "aws" = {
...
#+begin_src nix :noweb-ref account-setting :noweb yes
  {
    programs = {
      git = {
        enable = true;
      ...
      nix-direnv = super.nix-direnv.overrideAttrs (oldAttrs: rec {
        ...
        postPatch = ''
            substituteInPlace direnvrc \
              --replace "\''${NIX_BIN_PREFIX}" "\''${NIX_BIN_PREFIX:-${super.nix}/bin/}" \
      ...
      (
        ...
        in

          {
            ...
            home.packages = [ gmail-oauth2-tools ];
...
Register ~emacs~ with ~home-manager~
    ...
    in
      {
        nixpkgs.overlays = [
          (import (builtins.fetchTarball {
            url = "https://github.com/nix-community/emacs-overlay/archive/" + emacsOverlayHash + ".tar.gz";
          }))
        ];
...
I pin the commit of ~nix-community/emacs-overlay~ below in order to not randomly updating and rebuilding ~emacs~ when I run ~home-manager switch~.

#+begin_src nix :noweb-ref emacsOverlayVersion
  emacsOverlayHash = "4d5abf7952918493d448f7dabd24156624c821e3";
#+end_src

The official ~emacs-overlay~ has only Linux based ~emacs~. I modify it to include a ~emacs~ package that are compatible with macOS.

#+begin_src nix :noweb-ref emacs-package
  (
    let
      ...
      emacsNsGcc = pkgs.emacsGcc.overrideAttr (
        old: {
          configureFlags = (lib.remove "--with-xft" old.configureFlags)
                           ++ lib.singleton "--with-ns";
        }
      );

      emacsGccDarwin = pkgs.symlinkJoin {
        name = "emacsGccDarwin";
        paths = [ emacsNsGcc ];
    ...
    in
      (if pkgs.stdenv.isLinux then pkgs.emacsPgtkGcc else emacsGccDarwin)
  )
#+end_src
...
Install STATA with ~home-manager~.
...
~emacs-tree-sitter~ does not include ~R~ grammar. To build it from source, I will need install ~tree-sitter-cli~.
...
Check whether ~R~ grammar is downloaded and built. If not, pull ~tree-sitter-R~ from github and compile it.
...
Download and install ~tree-sitter~ Nix grammar
...
#+begin_src nix :noweb-ref home-manager
  (
    ...
    in
      ...
      {
        home.packages =  [ pkgs.texlive.combined.scheme-full digestif ];
      }
  )
#+end_src
...
Finally, I use ~Zathura~ as my pdf viewer.
...
Initializing system-specific configuration using the ~lib.mkIf~ function:
...
#+begin_src nix :noweb-ref flake-module
  drop-keyboard-loader-module = {
    home.packages = [
      (
        pkgs.stdenv.mkDerivation {
          name = "mdloader";
          src = drop-keyboard-loader;
          installPhase = ''
...
#+begin_src nix :noweb-ref linux-config :noweb yes
  (
...
#+begin_src nix :noweb-ref extra-linux-setup
  home.sessionPath = [ "/usr/bin" ];
#+end_src

In addition, I include some Linux-wayland specific packages.

#+begin_src nix :noweb-ref extra-linux-packages
  home.packages = with pkgs; [
    bemenu
    havoc # terminal
  ];
