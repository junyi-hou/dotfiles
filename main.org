...
My company laptop puts non-nix managed configurations into ~~/.bash_profile~. Therefore I would like to avoid making changes to this file. I write the following module overriding the default ~bash~ module in ~home-manager~ to achieve this.

#+begin_src nix :noweb-ref do-not-touch-bash_profile
  (
    { pkgs, lib, config, ... }: {
      config = (
      let
