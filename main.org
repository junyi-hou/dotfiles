...
~nix~ and ~nix-flakes~ can be installed via
...
Then add the following to ~/etc/nix/nix.conf~. Make sure to replace ~USER-NAME~ with the actual user name used in the system.
