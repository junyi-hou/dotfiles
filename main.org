...
Unfortunately, ~zathuraInverseSearch~ is system-specific. In Linux, I use ~ydotool~ to mimic mouse movement and ~swaymsg~ to detect current window setup. In MacOS, I use the builtin ~AppleScript~ for it.

# TODO: implement ~zathuraInverseSearch~
...
** implement ~zathuraInverseSearch~
