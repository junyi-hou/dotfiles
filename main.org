...
On MacOS system, display, sound and network modules are all builtin, so I don't need to worry about it.
...
I use a subflake to manage ~emacs~ and its packages. This flake defines a  function called ~emacsModule~ which takes an ~emacs~ derivation as the only argument, and returns a ~home-manager~ module with all ~emacs~-related configuration.
...
Below includes the structure of the subflake for all ~emacs~-related configuration.
...
To install ~emacs~ in Linux, call ~emacsModule~ function with the ~emacsGcc~ derivation from ~emacs-overlay~.
...
For MacOS, I need to wrap library around the default derivation and pass ~--with-ns~ flag when building ~emacs~ in order to use the ~cocoa~ API. I follow this [[https://github.com/twlz0ne/nix-gccemacs-darwin][repo]] for this task.
...
#+begin_src nix :noweb-ref common-modules
...
Allow the ~--no-tangle~ optional flag, which skips tangling ~.nix~ and ~.el~ from ~main.org~ and use the configuration as is.
