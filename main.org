...
To simplify this process, I define a function ~gatsby:core--switch-home~, which runs ~home-manager switch~ asynchronously and pop up a buffer ~*home-manager*~ to run and report the output from ~home-manager switch~. I then hook this function after ~org-babel-tangle~ finishes. To make sure that this process runs only after I tangle this file, I add a file-local variable ~gatsby:tangle-should-triggle-home-switch~, with value ~t~ in none but this file.

#+begin_src emacs-lisp :tangle no :noweb-ref home-manager-switch-hook
  (defcustom gatsby:tangle-should-triggle-home-switch nil
    "Non-nil means to run `gatsby:core--switch-home' after `org-babel-tangle'."
    :type 'boolean
    :group 'org)

  (put 'gatsby:tangle-should-triggle-home-switch 'safe-local-variable #'booleanp)

  (defun gatsby:core--switch-home (&rest _)
  ...
  This function should not bu ran manually."
    (when gatsby:tangle-should-triggle-home-switch
      ...
      (switch-to-buffer-other-window "*home-manager*")))

  (advice-add #'org-babel-tangle :after #'gatsby:core--switch-home)
#+end_src
...
+ ~mbsync~ (registered as ~isync~ in many package managers) to fetch and sync maildir with the remote server.
  ...
  #+begin_src nix :noweb account-setting
    {
      ...
      (
        ...
        in

          {
            ...
            home.packages = [ pkgs.isync gmail-oauth2-tools ];
...
Similarly, I advice ~org-gcal~ to close file after archiving old events.

#+begin_src emacs-lisp
  (defun gatsby:gcal--close-after-archive (fn &rest args)
    (cl-letf (((symbol-function #'save-buffer)
               (lambda () (save-buffer) (kill-buffer))))
      (apply fn args)))

  (advice-add #'org-gcal--archive-old-event :around #'gatsby:gcal--close-after-archive)
#+end_src
...
#+begin_src nix :noweb-ref home-manager
  {
    home.packages = with pkgs; [
      (python3.withPackages (p: with p; [ jupyter ]))
    ];
  }
#+end_src
