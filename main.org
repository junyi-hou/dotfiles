...
To simplify this process, I define a function ~gatsby:core--switch-home~, which runs ~home-manager switch~ asynchronously and pop up a buffer ~*home-manager*~ to run and report the output from ~home-manager switch~. I then hook this function after ~org-babel-tangle~ finishes. To make sure that this process runs only after I tangle *this* file, I add a file-local variable ~gatsby:is-config-source-file~, with value ~t~ in none but this file.
