...
- [[~emacs~ Package Management][~emacs~ daemon]]
- [[epa][gpg-agent]]
...
I need to swap ~option~ and ~command~, as ~option~ on MacOS works as ~alt~ but placed physical at where ~meta~ key usually is.

#+begin_src emacs-lisp :tangle no :noweb-ref extra-emacs-setting
...
#+begin_src nix :noweb-ref darwin-modules
  (
    let
      pkgs = import nixpkgs { inherit system; overlays = [ emacs-overlay.overlay ]; };
      macPort = pkgs.callPackage ./macos/macport/package.nix {
    ...
    in
      {
        imports = [
          ...
          (emacs-module.emacsPackage macPort)
        ];
      }
  )
#+end_src
...
On MacOS, ~emcas~ launched from ~launchd~ does not have ~$PATH~ set properly. ~exec-path-from-shell~ fix this issue.
...
I only want to enable it on Linux, as ~.gitignore~ and ~.gitconfig~ are managed by other parts on my MacOS laptop.
...
Here includes a series of small command line utilities. I include this setting only in Linux since My company laptop puts non-nix managed configurations into ~~/.bash_profile~, and enabling ~programs.bash~ would override this.
...
I include ~alacritty~ on Linux in case ~emacs~ fails.
