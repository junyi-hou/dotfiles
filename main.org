...
*** COMMENT incremental syntax tree parsing: tree-sitter

~tree-sitter~ provides a powerful and super fast incremental syntax parsing engine. ~emacs-tree-sitter~ implements ~tree-sitter~ parsing tools in ~emacs~.
...
To compile tree-sitter grammars for different languages, I need to install the tree-sitter command-line utility. The tree-sitter repository provides pre-built binary of the command-line utility. However, before [[https://github.com/NixOS/nix/pull/4153][this pull request]] gets merged, ~flake~ do not support using plain binary files as inputs. As a workaround, I have to manually specific their checksums when building them:

#+begin_src nix :tangle ./modules/tree-sitter.nix :mkdirp yes
  inputs: with inputs; { pkgs, ... }:
  let
...
Make ~tree-sitter CLI~ available in both Linux and MacOS.
...
*** COMMENT R:tree-sitter

~emacs-tree-sitter~ does not include ~R~ grammar. I need to compile ~R~ grammar from source. First check whether ~R~ grammar is downloaded and built. If not, pull ~tree-sitter-R~ from github and compile it.
...
*** COMMENT Nix:tree-sitter

Nix is not included in the grammar bundle provided by ~emacs-tree-sitter~. I need to download and compile it from source.
