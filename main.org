...
#+begin_src nix :tangle ./linux/use-system-wayland.nix :noweb yes
    home = {
      file = {
        ".config/fontconfig" = {
...
Make sure to load those configuration on the correct system.

#+begin_src nix :noweb-ref home-manage
  imports = [
    (if stdenv.isLinux then ./linux/use-system-wayland.nix)
  ];
#+end_src
