...
To make this configuration portable, I define a font specifically for ~emacs~ using ~home-manager~. The vanilla ~home-manager~ do not have a module to install fonts. There is a [[https://github.com/nix-community/home-manager/issues/520#issuecomment-451759097][comment]] that adds a custom font module to ~home-manager~, which I include below.
...
To install fonts, first register the source file in ~flake~.
...
Then install it using the custom module.
