...
*** unified documentation-displaying mechanism: help-childframe
...
Use ~emacs-overlay~ from ~nix-community~ to obtain the latest git version of ~emacs~.

#+begin_src nix :noweb-ref flake-input
  emacs-overlay.url = "github:nix-community/emacs-overlay";
#+end_src

#+begin_src nix :tangle ./modules/emacs.nix :mkdirp yes
  inputs: with inputs; {
...
For ~linux~ system, ~emacs-overlay~ provides derivation for ~native-comp~ version, so I can just use it.
...
For ~macOS~, I follow the method used [[https://github.com/twlz0ne/nix-gccemacs-darwin][here]] to enable ~cocona~ and wrap ~emacsGit~ with ~macOS~ specific library path.
...
Enable ~emacs~ in both supported platforms.
...
** ein: freezes ~emacs~
** replace straight.el with nix-flake
check out https://github.com/vlaci/nix-straight.el
